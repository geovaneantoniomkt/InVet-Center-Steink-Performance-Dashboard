/* Dashboard InVet Center — lógica de apresentação (sem dependências externas).
...
/** "1 contato" / "2 contatos" — evita o "1 contatos". */
...
const OBJECTIVE_LABEL = {
  OUTCOME_LEADS: "Cadastros / conversas", OUTCOME_SALES: "Vendas / conversões", OUTCOME_TRAFFIC: "Tráfego", OUTCOME_AWARENESS: "Reconhecimento",
...
function loadTargets() {
  ...
  try {
    const saved = JSON.parse(localStorage.getItem("invet-center.targets") || "null");
    if (saved && typeof saved === "object") return { ...base, ...saved };
  } catch { /* ignore */ }
  return base;
}
function saveTargets(t) { try { localStorage.setItem("invet-center.targets", JSON.stringify(t)); } catch { /* ignore */ } }
...
/* Meta Ads não trabalhado (meta.json com configured:false): as métricas aparecem
   zeradas, mas sem inventar saldo, status nem número de conta. */
const mEnabled = () => !!(state.meta && state.meta.configured !== false);
/* Receita e ROAS só fazem sentido quando a plataforma recebe valor real de venda.
   Em serviço (hospital, clínica) o "valor da conversão" é um peso interno da conta:
   exibir como receita geraria um ROAS falso. Controlado em config.json → display.revenue. */
const revenueOn = () => state.config?.display?.revenue !== false;
/* Categorias de conversão do Google Ads agrupadas pelo canal de contato.
   Robusto a renomeação das ações na conta — a categoria é do próprio Google. */
const G_CALL_CATS = ["PHONE_CALL_LEAD", "PHONE_CALL"];
const G_CONTACT_CATS = ["CONTACT", "SUBMIT_LEAD_FORM", "BOOK_APPOINTMENT", "REQUEST_QUOTE", "SIGNUP", "LEAD"];
function gConvByCat(rows, cats) {
  let s = 0;
  for (const r of rows) if (cats.includes(r.category)) s += r.conversions || 0;
  return s;
}
...
function renderExec() {
  ...
  /* contatos do Google separados por canal — ligação é decisiva num pronto-socorro 24h */
  const gConvRows = gEnabled() ? gRowsIn(state.since, state.until, state.google.conversions_daily || []) : [];
  const gConvPrevRows = gEnabled() ? gRowsIn(state.prevSince, state.prevUntil, state.google.conversions_daily || []) : [];
  const calls = gConvByCat(gConvRows, G_CALL_CATS), callsPrev = gConvByCat(gConvPrevRows, G_CALL_CATS);
  const gContacts = gConvByCat(gConvRows, G_CONTACT_CATS), gContactsPrev = gConvByCat(gConvPrevRows, G_CONTACT_CATS);
  ...
  const totalRevenue = revenueOn() ? all.revenue + g.conversions_value : 0;
  ...
  const gLifetime = gEnabled() ? sum(state.google.daily || [], (r) => r.cost || 0) : 0;
  const gActive = gEnabled() ? (state.google.campaigns || []).filter((c) => c.status === "ENABLED").length : 0;
    ...
    mEnabled()
      ? h("div", { class: "tile" },
        ...
        h("div", { class: "sub faint" }, gEnabled()
          ? `Google Ads ${state.config.google_ads.customer_id}: ${gActive} campanhas ativas`
          : `Google Ads ${state.config.google_ads.customer_id}: aguardando credenciais da API`),
      )
      : h("div", { class: "tile" },
        h("div", { class: "label", text: "Contas de anúncios" }),
        h("div", { class: "value", text: fmt.brl(gLifetime) }),
        h("div", { class: "sub", text: `investido no Google Ads desde ${fmt.date(state.google?.range?.since || state.since)}` }),
        h("div", { class: "sub" }, gEnabled() ? badge("ok", "Google ativo") : badge("crit", "Google sem integração"),
          badge("neutral", "Meta sem investimento")),
        h("div", { class: "sub faint", text: gEnabled() ? `${gActive} campanhas ativas · conta ${state.config.google_ads.customer_id} · fuso ${m.account.timezone}` : "" }),
        h("div", { class: "sub faint", text: "Meta Ads: nenhuma campanha e nenhum investimento no período" }),
      ),
  );
  ...
  const kpis = h("div", { class: "grid c6" },
    ...
    gEnabled()
      ? tile({
        ...
        sub: h("span", {}, `${fmt.brl(div(totalSpend, all.purchases + g.conversions))} cada · meta ${fmt.brl(T.google_cost_per_conversion)} `,
          g.cost > 0 ? badge(gAssess(g).level, levelWord(gAssess(g).level)) : null),
      })
      : tile({
    ...
    totalRevenue > 0
      ...
      : tile({
        label: "Ligações", value: fmt.int(Math.round(calls)),
        delta: deltaBadge(calls, callsPrev),
        sub: g.conversions > 0
          ? `${fmt.dec1((calls / g.conversions) * 100)}% dos contatos · chamadas a partir do anúncio`
    ...
    tile({
      ...
      sub: all.conversations > 0 ? `${fmt.brl(all.cost_per_conversation)} por conversa` : "sem campanhas de mensagem no Meta Ads",
    }),
    mEnabled()
      ? tile({
      ...
      /* sem Meta não existe alcance (pessoas únicas): no Google a base é impressão */
      : tile({
        label: "Impressões e cliques", value: fmt.compact(g.impressions),
        delta: deltaBadge(g.impressions, gPrev.impressions),
        sub: `${fmt.int(g.clicks)} cliques · CTR ${fmt.pct(g.ctr)} · CPC ${fmt.brl(g.cpc)}`,
      }),
  );
  ...
  const resultSeries = [{ name: "Contatos (Meta)", color: COLOR.meta, values: days.map((d) => byDayAll.get(d)?.purchases || 0) }];
  ...
  const charts = h("div", { class: "grid c3" },
    ...
    chartCard("Resultados por dia", "contatos por plataforma", resultSeries, { labels, type: "bar", stacked: true }),
    chartCard("Custo por contato, por dia", mEnabled() ? "gasto em conversão ÷ contatos do dia" : "investimento do Google ÷ contatos do dia",
      [{
        name: "Custo por contato", color: COLOR.warn,
        values: days.map((d) => (mEnabled() ? (byDayCap.get(d)?.cost_per_purchase ?? null) : (gByDay.get(d)?.cpa ?? null))),
      }],
      { labels, type: "line", format: fmt.brl, target: mEnabled() ? T.cost_per_purchase : T.google_cost_per_conversion }),
  );
  ...
  const gCallsByMonth = new Map();
  if (gEnabled()) {
    for (const r of state.google.conversions_daily || []) {
      if (!G_CALL_CATS.includes(r.category)) continue;
      const k = r.date.slice(0, 7);
      gCallsByMonth.set(k, (gCallsByMonth.get(k) || 0) + (r.conversions || 0));
    }
  }
  ...
  const totals = { spend: 0, meta: 0, google: 0, gconv: 0, purchases: 0, capSpend: 0, revenue: 0, calls: 0, conversations: 0, impressions: 0, clicks: 0 };
  const monthRowsEls = shownKeys.map((k) => {
    ...
    const rev = revenueOn() ? a.revenue + gm.conversions_value : 0;
    ...
    const contatos = a.purchases + gm.conversions;
    const mCalls = gCallsByMonth.get(k) || 0;
    totals.purchases += contatos; totals.capSpend += c.spend + gm.cost; totals.revenue += rev;
    totals.calls += mCalls; totals.conversations += a.conversations;
    ...
    return h("tr", { class: partial ? "parcial" : "" },
      ...
      h("td", { text: fmt.dec1(contatos) }), h("td", { text: fmt.brl(div(c.spend + gm.cost, contatos)) }),
      h("td", { text: rev > 0 ? fmt.brl(rev) : "—" }), h("td", { text: rev > 0 ? fmt.x(div(rev, spend)) : "—" }),
      h("td", { text: gEnabled() ? fmt.dec1(mCalls) : "—" }), h("td", { text: fmt.int(a.conversations) }),
  ...
  const totalRow = shownKeys.length > 1
    ? h("tr", { class: "total" },
      ...
      h("td", { text: fmt.dec1(totals.purchases) }), h("td", { text: fmt.brl(div(totals.capSpend, totals.purchases)) }),
      h("td", { text: totals.revenue > 0 ? fmt.brl(totals.revenue) : "—" }), h("td", { text: totals.revenue > 0 ? fmt.x(div(totals.revenue, totals.spend)) : "—" }),
      h("td", { text: gEnabled() ? fmt.dec1(totals.calls) : "—" }), h("td", { text: fmt.int(totals.conversations) }),
  ...
  const histTable = h("div", { class: "table-wrap" }, h("table", {},
    h("thead", {}, h("tr", {},
      ...
      h("th", { text: "Contatos" }), h("th", { text: "Custo/contato" }), h("th", { text: "Receita" }), h("th", { text: "ROAS" }),
      h("th", { text: "Ligações" }), h("th", { text: "Conversas" }), h("th", { text: "Impressões" }), h("th", { text: "Cliques" }), h("th", { text: "CTR" }), h("th", { text: "CPC" }))),
...
function buildAlerts({ rows, cap, prevCap, all, prevAll, campRows, gCampRows, adRows, projection, budget, monthSpend, capLeft, g }) {
  ...
  if (ok(cap.cost_per_purchase) && ok(prevCap.cost_per_purchase) && prevCap.purchases >= 2 && cap.cost_per_purchase > prevCap.cost_per_purchase * 1.25) {
    out.push({ level: "warn", title: `Custo por contato subiu ${nfDec1.format(((cap.cost_per_purchase / prevCap.cost_per_purchase) - 1) * 100)}% vs. período anterior`,
...
function renderMeta() {
  ...
  const head = h("div", { class: "grid c4" },
    ...
    tile({
      label: "Conversas no WhatsApp", value: fmt.int(cap.conversations),
      sub: cap.conversations > 0 ? `${fmt.brl(cap.cost_per_conversation)} por conversa` : "sem campanhas de mensagem no Meta Ads",
    }),
  );
  ...
  const bdTable = (title, sub, label, list) => h("div", { class: "table-wrap" },
    ...
    h("table", {}, h("thead", {}, h("tr", {}, h("th", { text: label }), h("th", { text: "Gasto" }), h("th", { text: "Contatos" }), h("th", { text: "Custo/contato" }), h("th", { text: "Conversas" }), h("th", { text: "CTR" }))),
      h("tbody", {}, ...list.map(([k, a]) => h("tr", {}, h("td", { text: k }), h("td", { text: fmt.brl(a.spend) }), h("td", { text: fmt.int(a.purchases) }),
        h("td", { text: fmt.brl(a.cost_per_purchase) }), h("td", { text: fmt.int(a.conversations) }), h("td", { text: fmt.pct(a.ctr) }))))));
  ...
  /* filter(Boolean): append(null) insere o texto "null" na página */
  sec.append(...[
    block("Meta Ads", `${fmt.date(state.since)} a ${fmt.date(state.until)} · meta de ${fmt.brl(T.cost_per_purchase)} por contato`, head, table),
      ...
      block("Como conectar", "uma vez só — depois a atualização é automática todo dia às 7h",
        h("div", { class: "setup" },
          ...
          h("div", { class: "card" },
            ...
            h("ol", {},
              h("li", {}, "Na conta, marcar como ", h("b", { text: "conversão principal" }), " só o que é contato de verdade: WhatsApp, ligação ou formulário."),
  ...
  const table = h("div", { class: "table-wrap" }, h("table", {},
    ...
    h("tbody", {}, ...campRows.map(({ c, agg, prev: p }) => {
      ...
      return h("tr", {},
        ...
        h("td", {}, gCprCell(agg)),
        h("td", { text: revenueOn() && agg.conversions_value > 0 ? fmt.brl(agg.conversions_value) : "—" }),
        h("td", { text: revenueOn() && agg.conversions_value > 0 ? fmt.x(agg.roas) : "—" }),
        h("td", { text: fmt.int(agg.impressions) }),
        h("td", { text: fmt.int(agg.clicks) }),
        h("td", { text: fmt.pct(agg.ctr) }),
        h("td", { text: fmt.brl(agg.cpc) }),
...
function renderCreatives() {
  ...
  sec.append(block("Criativos com entrega", "clique em “abrir” para ver o anúncio publicado", filters,
    items.length ? cards : h("div", { class: "empty", text: mEnabled() ? "Nenhum criativo para este filtro." : "Nenhum criativo — não há campanhas de Meta Ads no período." })));
}
...
function renderOrganic() {
  ...
  const semDados = () => block("Orgânico", "Instagram e Facebook", h("div", { class: "placeholder" },
    h("b", { text: "Sem dados de Instagram e Facebook" }),
    h("span", { text: "As contas orgânicas da InVet Center ainda não estão integradas ao dashboard." })));
  if (!o) { sec.append(semDados()); return; }
  const ig = o.instagram, fb = o.facebook;
  if (!ig && !fb) { sec.append(semDados()); return; }
...
function renderLegend() {
  ...
  sec.append(
    block("Métricas", "", h("div", { class: "card" }, dl([
      ["Contato", "Resultado principal da InVet Center: um tutor que pediu atendimento a partir do anúncio — formulário do site, clique no WhatsApp ou ligação."],
      ...
      ["Ligação", "Chamada iniciada direto do anúncio (categoria “ligação” do Google Ads). Num pronto-socorro 24h é o contato mais quente: quem liga quer atendimento agora."],
      ["Contato no site e WhatsApp", "Formulário do site, clique no botão de WhatsApp e botão flutuante — as três ações marcadas como conversão principal na conta."],
      ["Receita e ROAS", "Só existe quando a plataforma recebe o valor da venda. O hospital não envia valor por atendimento, por isso essas colunas ficam com “—”."],
      ...
      ["Conversão (Google)", "Ação marcada como conversão principal na conta do Google Ads. Na InVet Center são três: contato pelo site, clique no WhatsApp e ligação."],
      ["Conversão vs. topo de funil", "Conversão = campanhas de venda, cadastro e conversa. Topo = tráfego, alcance, engajamento e visitas ao perfil. O custo por contato usa só a conversão."],
    ...
    block("Fontes e atualização", "", h("div", { class: "card" }, dl([
      ["Meta Ads", mEnabled()
        ? `Marketing API ${state.meta.api_version} · conta ${state.meta.account.id} (${state.meta.account.name}) · série diária por anúncio desde ${fmt.date(state.meta.range.since)}.`
        : "Sem conta de Meta Ads integrada ao dashboard — nenhum investimento no período, por isso as métricas de Meta Ads aparecem zeradas."],
      ["Google Ads", gEnabled()
      ...
      cfg.meta.page_id || cfg.instagram.username
        ...
        : ["Orgânico", "Instagram e Facebook ainda não integrados ao dashboard."],
      cfg.meta.pixel_id
...
function renderHeader() {
  ...
  $("#topbar-sub").textContent = `${c.segment} · ${gEnabled() ? "Google Ads, " : ""}Meta Ads e orgânico`;
  ...
  $("#hdr-updated").textContent = fmt.dateTime(state.google?.generated_at || state.meta.generated_at);
  const { first, last } = dataBounds();
...
async function boot() {
  ...
  $("#btn-reset-targets").addEventListener("click", () => {
    try { localStorage.removeItem("invet-center.targets"); } catch { /* ignore */ }
