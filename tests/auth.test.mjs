...
const SITE = "https://invet-center-dashboard.netlify.app";
...
for (const target of targets) {
  ...
  const cookieOf = (res) => ((res.headers.get("set-cookie") || "").match(/ic_sess=([^;]*)/) || [])[1] || null;
  ...
  /* --------------------------------------------------------------- com sessão */
  const auth = { cookie: `ic_sess=${sess}` };
  r = await get("/", auth);
  ...
  r = await get("/", { cookie: `ic_sess=${payload}.assinaturaFalsa` });
  ...
  r = await get("/", { cookie: `ic_sess=${forjado}.${signature}` });
  ...
  r = await get("/", { cookie: `ic_sess=${expirado}.${signature}` });
